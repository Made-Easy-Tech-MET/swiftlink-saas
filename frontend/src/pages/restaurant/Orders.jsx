...
export default function Orders() {
  ...
  const getNextActions = (currentStatus) => {
    const actions = {
      pending: [
        { label: 'Confirm', status: 'confirmed', color: 'primary' },
        { label: 'Cancel', status: 'cancelled', color: 'danger' }
      ],
      confirmed: [
        { label: 'Preparing', status: 'preparing', color: 'primary' }
      ],
      preparing: [
        { label: 'Ready for Pickup', status: 'ready', color: 'primary' }
      ],
      ready: [],
      picked_up: [],
      delivered: [],
          ...
          <div className="flex items-center gap-2">
            {nextActions.map((action) => (
              <Button
                ...
                size="sm"
                onClick={() => handleUpdateOrderStatus(row.id, action.status)}
              >
                {action.label}
              </Button>
            ))}
